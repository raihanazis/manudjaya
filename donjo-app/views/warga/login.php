							...
							<?php else : ?>
								...
								<?php else : ?>
									<?php if ($this->session->daftar) : ?>
										<!-- View Pendaftaran -->
										<form id="validasi" action="<?php echo base_url(); ?>index.php/login/proses-daftar" method="post" class="login-form" enctype="multipart/form-data">
											<?php $this->load->view(WARGA . '/pendaftaran') ?>
										</form>
									<?php else : ?>
